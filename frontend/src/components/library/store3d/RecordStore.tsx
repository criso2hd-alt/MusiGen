...
function Room({tracks, label, quality, visit, onExit, exitRef}: {tracks:Track[]; label:string; quality:'balanced'|'low'; visit:Visit; onExit:()=>void; exitRef:RefObject<(()=>void)|null>}) {
  ...
  return <div className="vinyl-room">
    ...
    {inspection && <div className="vinyl-inspect-info"><span className="vinyl-kicker">NOW IN YOUR HANDS</span><strong>{inspection.title}</strong><p>{Math.floor(inspection.duration/60)}:{Math.floor(inspection.duration%60).toString().padStart(2,'0')} · Seed {inspection.seed}</p><p>Move mouse to rotate when captured, or drag the sleeve.</p><div className="vinyl-actions"><button onClick={()=>controls.current?.flip()}>Flip sleeve</button><button onClick={()=>controls.current?.play()}>Play record</button><button onClick={()=>controls.current?.inspect()}>Return record</button></div></div>}
    {!captured && !inspection && !error && !freeLook && <div className="vinyl-entry"><span className="vinyl-kicker">TAKE YOUR TIME. FIND YOUR SOUND.</span><h2>Stay for one more track.</h2><p>Wander the aisles. Every sleeve is a song from your collection.</p><button onClick={()=>controls.current?.capture()}>Enter / capture mouse</button><button onClick={()=>{setFreeLook(true);controls.current?.freeLook();}}>Browse without capture</button><small>WASD to walk · Mouse to look · Shift toggles capture · Esc releases</small></div>}
    <div className="vinyl-help">{inspection?'Click / F returns sleeve · Right click / Space plays':'WASD walk · Mouse look · Left click / F inspect · Right click / Space play'}<span>{freeLook && !captured?"Drag to look · ":""}Shift — {captured?'release':'capture'} mouse · Esc — release</span></div>
